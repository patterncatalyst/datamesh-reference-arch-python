...
A §17 capstone data product. Owns the `reviews` Postgres schema
...
```bash
# from examples/lgtm-datamesh/
./scripts/build-image.sh services/review-service review-service v1
helm upgrade --install review-service charts/capstone/charts/review-service -n capstone
kubectl rollout status deployment/review-service -n capstone --timeout=120s
```
