...
#   - order-service-openapi  (OPENAPI)  ← fetched from order-service /q/openapi?format=json
#   - inventory-grpc-proto   (PROTOBUF) ← read from the committed .proto file
#   - graphql-gateway-sdl    (GRAPHQL)  ← fetched from the gateway /graphql/schema.graphql endpoint
#
...
print("==> OpenAPI (order-service /q/openapi?format=json)")
openapi = fetch(f"{order_url}/q/openapi?format=json")
...
print("==> GraphQL SDL (gateway /graphql/schema.graphql)")
sdl = fetch(f"{gateway_url}/graphql/schema.graphql", as_text=True)
