...
for _ in $(seq 1 15); do
    curl -s -o /dev/null --max-time 2 "http://127.0.0.1:${PORT}/q/health/live" && break
    sleep 1
done

base="http://127.0.0.1:${PORT}"

step "GET /q/health/live + /q/health/ready + /version"
curl -fsS --max-time 5 "${base}/q/health/live"  | grep -q '"status":"UP"'    || fail "/q/health/live not ok"
curl -fsS --max-time 5 "${base}/q/health/ready" | grep -q '"status":"UP"' || fail "/q/health/ready not ready (Postgres?)"
