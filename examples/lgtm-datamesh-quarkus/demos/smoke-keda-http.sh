...
#
# Load is driven at /q/health/live (liveness, no datastore) so the demo doesn't depend
...
CODE=$(curl -s -o /dev/null -w '%{http_code}' --max-time 200 \
    -H "Host: $HOST" "http://127.0.0.1:${LOCAL_PORT}/q/health/live" || echo "000")
...
for _ in $(seq 1 8); do
    ( for _ in $(seq 1 40); do
        curl -s -o /dev/null --max-time 10 -H "Host: $HOST" "http://127.0.0.1:${LOCAL_PORT}/q/health/live" || true
      done ) &
    LOAD_PIDS+=($!)
done
