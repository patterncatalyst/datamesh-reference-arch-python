...
#   4. wait for rollout, then assert GET /q/health/live and GET /q/health/ready
...
step "Assert GET /q/health/live returns ok"
H="$(curl -fsS "http://127.0.0.1:${LOCAL_PORT}/q/health/live")" || fail "/q/health/live did not return 200"
echo "    $H"
echo "$H" | grep -q '"status":"UP"' || fail "/q/health/live body unexpected: $H"
# Quarkus SmallRye Health does not echo the service name in the body (the
# Python /health did); the readiness DB check below is the meaningful assertion.

step "Assert GET /q/health/ready reports readiness (Postgres reachable)"
Z="$(curl -fsS "http://127.0.0.1:${LOCAL_PORT}/q/health/ready")" || fail "/q/health/ready did not return 200 (Postgres unreachable?)"
echo "    $Z"
echo "$Z" | grep -q '"status":"UP"' || fail "/q/health/ready not ready: $Z"
