...
step "Assert /q/health/live returns ok"
health=$(curl -fsS "$BASE/q/health/live") || fail "/q/health/live unreachable"
echo "$health" | grep -q '"status":"UP"' || fail "/q/health/live did not return ok: $health"

step "Assert /q/health/ready reports readiness (Postgres reachable)"
healthz=$(curl -fsS "$BASE/q/health/ready") || fail "/q/health/ready returned non-2xx (DB unreachable?)"
echo "$healthz" | grep -q '"status":"UP"' || fail "/q/health/ready not ready: $healthz"
